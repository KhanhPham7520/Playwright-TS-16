...
async function selectTreeItemByLabel(label: string, inputs: string[], page: Page) {
    const treeSelectXpath = `//div[@role='separator' and normalize-space()='${label}']/following::input[@role='combobox'][1]`;
    await page.locator(treeSelectXpath).click();
    for (let i = 0; i < inputs.length; i++) {
        if (i === inputs.length - 1) {
            const treeNodeXpath = `//span[contains(concat(' ', @class, ' '), ' ant-select-tree-node-content-wrapper ') and normalize-space()='${inputs[i]}']`;
            await page.locator(treeNodeXpath).click();
        } else {
            const treeSwitcherXpath = `//span[normalize-space()='${inputs[i]}']/preceding::span[contains(concat(' ', @class, ' '), ' ant-select-tree-switcher ')][1]`;
            let treeSwicherLocator = page.locator(treeSwitcherXpath);
            let currentClasses = await treeSwicherLocator.getAttribute('class');
